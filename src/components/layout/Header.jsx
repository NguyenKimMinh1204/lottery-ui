...
//         {/* Logo */}
//         <a href="/" className="flex-shrink-0 transition-transform hover:">
//           <img 
...
//               <a href="#" className="block font-bold px-6 py-3 text-gray-700 hover:bg-clip-text hover:text-transparent hover:bg-gradient-to-l hover:from-[#cd0394] hover:via-[#6127bf] hover:to-[#3e1074] transition-all duration-300">
//                 Câu hỏi thường gặp
//               </a>
//               <a href="#" className="block font-bold px-6 py-3 text-gray-700 hover:bg-clip-text hover:text-transparent hover:bg-gradient-to-l hover:from-[#cd0394] hover:via-[#6127bf] hover:to-[#3e1074] transition-all duration-300">
...
//             className="relative w-12 h-12 rounded-full bg-gradient-to-br from-gray-100 to-gray-200 border-2 border-gray-300 hover:border-purple-600 hover:  duration-300 shadow-md hover:shadow-lg flex items-center justify-center"
//             aria-label="Giỏ hàng"
//           >
...
const menuItems = [
  ...
  { label: 'Gói cước', href: '/' },
  {
    ...
    subItems: [
      ...
      { label: 'Điểm cung cấp dịch vụ', href: '#' },
    ],
  },
  {
    label: 'Giới thiệu',
    subItems: [{ label: 'Tin tức', href: '#' }],
  },
];
...
export default function Header({ setIsSidebarOpen, setIsCartOpen, cartCount = 0 }) {
  return (
    <header className="fixed top-0 left-0 right-0 z-50 bg-gradient-to-br from-white to-gray-50 shadow-lg border-b border-gray-200">
      <div className="container mx-auto px-4">
        <nav className="flex items-center justify-between h-[74px] text-[13px]">
          {/* Logo */}
          <a href="/" className="flex-shrink-0">
            <img src="/logo.2d2d0d2c.svg" alt="Logo" className="w-[150px] h-[74px]" />
          </a>
            ...
            {menuItems.map((item) =>
              item.subItems ? (
                <div key={item.label} className="relative group">
                  ...
                  <div className="absolute top-full left-0 mt-2 min-w-[220px] bg-white rounded-2xl shadow-xl opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-300 overflow-hidden">
                    {item.subItems.map((sub) => (
                      <a
                        key={sub.label}
                        href={sub.href}
                        className="block font-bold px-6 py-3 text-gray-700 hover:bg-clip-text hover:text-transparent hover:bg-gradient-to-l hover:from-[#cd0394] hover:via-[#6127bf] hover:to-[#3e1074] transition-all duration-300"
                      >
                        {sub.label}
                      </a>
                    ))}
                ...
                <a
                  key={item.label}
                  href={item.href}
                  className="px-5 py-3 text-gray-700 font-semibold rounded-full hover:text-blue-500 transition-all duration-300 relative group"
                >
                  {item.label}
                  <span className="absolute bottom-0 left-1/2 -translate-x-1/2 w-0 h-0.5 bg-blue-500 group-hover:w-4/5 transition-all duration-300"></span>
                </a>
          ...
          <div className="hidden lg:flex items-center gap-3">
            ...
            <button
              onClick={() => setIsCartOpen && setIsCartOpen(true)}
              className="relative w-12 h-12 rounded-full bg-gradient-to-br from-gray-100 to-gray-200 border-2 border-gray-300 hover:border-purple-600 duration-300 shadow-md hover:shadow-lg flex items-center justify-center"
              aria-label="Giỏ hàng"
            >
              <svg className="w-5 h-5 text-gray-700" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path
